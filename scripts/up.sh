...
for n in "${NODES[@]}"; do
  ssh admin@"$n" "cd ~/compose/mentatd && sudo docker compose -f mentatd.yaml up -d" &
done; wait

echo "== mentatd-serve (OpenAI front door, :6381) on the head =="
ssh admin@"$HEAD" "cd ~/compose/mentatd-serve && sudo docker compose -f mentatd-serve.yaml up -d"

echo "== vLLM on every node =="
for n in "${NODES[@]}"; do
  ssh admin@"$n" "cd ~/$COMPOSE_DIR && sudo docker compose $FILES up -d" &
done; wait
...
echo "timed out; check: ssh admin@$HEAD 'sudo docker logs glm53 | tail -40'" >&2
exit 1
