...
def t6a_impl(R, cap_lens, replays):
    ...
    for lens in replays:
        ...
        worst = max(worst, rel)
        assert not nan
    print(f"T6a R={R} cap={cap_lens} worst replay rel err {worst:.4g}")
    assert worst < 0.1


@section("T6a SM90 MLA wrapper: capture at dummy lens, replay at real lens")
def t6a():
    t6a_impl(8, list(range(1, 9)), [list(range(1, 9)), list(range(100, 108)), list(range(693, 701)), list(range(2044, 2052))])
    # spec-off production shape: one request, one token, dummy seq_len 1
    t6a_impl(1, [1], [[1], [100], [700], [2051], [100], [37], [2600 // 4 * 4 + 3], [5]])
    # MTP k=3 shape: one request, 4 tokens, dummy seq_lens 1..4
    t6a_impl(4, [1, 2, 3, 4], [[1, 2, 3, 4], [100, 101, 102, 103], [700, 701, 702, 703], [2049, 2050, 2051, 2052], [40, 41, 42, 43]])
