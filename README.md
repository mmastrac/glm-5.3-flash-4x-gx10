...
is a stock vLLM nightly plus a handful of small patches. Ray is replaced by
[mentat](https://github.com/mmastrac/mentat).
...
| `compose/mentatd.yaml` | the mentat daemon, one per box |
| `compose/mentatd-serve.yaml` | the router clients talk to, on one box |
| `.env.example` | per-host and per-box values; copy to `compose/.env` |
| `smoketest/` | `run.sh <base> [served-name]` |
| `scripts/` | `up.sh` and `down.sh`, which drive all four boxes over ssh |
...
The build compiles nothing. The base is
`vllm/vllm-openai:nightly-0961bbae`, which carries `glm5_next` and DFlash2
upstream, and the patches are small anchored edits that fail the build if the
tree moves under them. `image/verify-base.py` then checks the finished tree.
The image embeds mentat 0.12.0, which refuses daemons older than 0.9, so
`mentatd` and `mentatd-serve` must be 0.12.0 too.
...
Compose reads `.env` from `compose/`, beside the compose files, not from the
directory you run it in. All three compose files read the same one.
...
It takes its identity from `VLLM_HOST_IP` and joins the mesh through
`HEAD_HOST`, so it needs nothing beyond the `.env` above. The model container
registers with it on `127.0.0.1:6379`. Then, on the head only:

    docker compose -f compose/mentatd-serve.yaml up -d

`compose/mentatd.yaml` explains the optional settings: interface ranking and
...
`scripts/up.sh <head> <worker> <worker> <worker>` and `scripts/down.sh
<box>...` do steps 5 and 6 from a workstation over ssh, in this order, and
`up.sh` refuses to start while any `glm53` container remains. They expect the
repo at `~/glm-5.3-flash-4x-gx10` on each box (`REPO_DIR`) and use `sudo
docker`.
...
| `glm53_kpool_tail_ring.py` | The kpool indexer's tail ring held 4 slots, but a speculative verify step writes 1+k rows (8 with DFlash2) before acceptance. A rejected draft then left a wrong indexer key for the rest of the request, which changes attention past 2048 tokens. The ring now covers a spec step. | ours, the ring part of [vllm#55219](https://github.com/vllm-project/vllm/pull/55219) |
...
`_glm47_failclosed_test.py` and `_glm53_dflash2_kv_groups_test.py` beside
them are tests for two of the patches; the image does not use them. The old
recipe's `gb10_topk_fallback.py` is now a flag
