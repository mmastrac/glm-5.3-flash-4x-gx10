...
| `thinking_budget_guard.py` | `thinking_token_budget` is applied inside `apply_sampling_params`, which the V2 sampler skips unless `_requires_logits_processing()` is true — and that predicate never checks the budget. Temperature 0 (greedy) and temperature 1 (this model's `generation_config` default) miss every other condition, so the budget is silently ignored on the most common requests. Measured at budget 16: temp 0 → 1426 chars, temp 1.0 → 1996, temp 0.6 → 61. Adds the missing condition; upstream main fixes the same gap with a per-request `needs_logits_processing` flag. Note the guard is `np.any` over the batch, so before this a co-scheduled request with an unusual temperature turned the cap on for everyone — which is why it looked intermittent rather than broken. | ours, after [vllm#50473](https://github.com/vllm-project/vllm/issues/50473) |
...
**`thinking_token_budget` does nothing.** On a stock image it is silently
skipped on the V2 runner whenever `temperature` is 0 or 1 — see Patches. This
recipe fixes it at build time; on an unpatched image, send `temperature`
outside {0, 1} as a workaround.
