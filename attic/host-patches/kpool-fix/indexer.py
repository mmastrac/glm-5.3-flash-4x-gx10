...
class KpoolTailMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(
        ...
        # TAIL-SLOT-FIX: the indexer op is NOT an eager break under a FULL
        # cudagraph (eager_break_during_capture returns fn() inside the
        # capture when cudagraph_runtime_mode == FULL), so the kpool decode
        # kernel captures the address of whatever tensor holds the tail slot
        # mapping. A per-step clone is freed after each step and its address
        # is reused, so replays read stale or foreign data as tail slots.
        # Hand the kernel one persistent buffer, refreshed in place per step.
        self._tail_slot_buffer = torch.zeros(
            vllm_config.scheduler_config.max_num_batched_tokens
            + vllm_config.scheduler_config.max_num_seqs * 16,
            dtype=torch.int64,
            device=device,
        )
        self._tail_slot_buffer_dtype_set = False
    ...
    def build(
        ...
        if positions is not None:
            ...
            # TAIL-SLOT-FIX: copy into the persistent buffer (same dtype as
            # the group's slot_mapping so the kernel's int32 load is unchanged).
            if not self._tail_slot_buffer_dtype_set:
                if self._tail_slot_buffer.dtype != fresh.dtype:
                    self._tail_slot_buffer = self._tail_slot_buffer.to(fresh.dtype)
                self._tail_slot_buffer_dtype_set = True
            n = fresh.shape[0]
            assert n <= self._tail_slot_buffer.shape[0], (
                f"tail slot buffer too small: {n} > {self._tail_slot_buffer.shape[0]}"
            )
            self._tail_slot_buffer[:n].copy_(fresh)
            slot_mapping = self._tail_slot_buffer[:n]
