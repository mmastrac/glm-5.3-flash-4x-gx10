...
def init_speculator(vllm_config: VllmConfig, device: torch.device):
    ...
    if speculative_config.method == "dflash":
        # DFLASH2-PATCH: a DFlash2 checkpoint keeps method "dflash" (same
        # query layout, same worker) but swaps the per-slot argmax for the
        # candidate path selector. Keyed on the architecture the drafter's
        # config.json declares; EAGLEConfig keeps names starting with "DFlash".
        if "DFlash2DraftModel" in speculative_config.draft_model_config.architectures:
            from vllm.v1.worker.gpu.spec_decode.dflash.dflash2_speculator import (
                DFlash2Speculator,
            )
