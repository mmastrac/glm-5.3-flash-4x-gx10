...
def _get_kv_cache_groups_glm5_next(
    ...
    # MLA(+kpool indexer)+mamba spec set. They used to knock this grouper out
    # (`type(s) is MLAAttentionSpec` for every attention layer), which sent the
    # whole model down the generic page-size unifier, where the kpool indexer
    # page (storage_block * 132 B) can never divide the MLA page
    # (block * 512 B) for any block size -- and which would have lost the
    # slot-shared tensor layout below anyway. Carve them out here and give
    # them their own group(s) with per-layer tensors (see
    # _glm5_next_draft_groups); everything about the target layout is unchanged.
    draft_specs = {
        k: v for k, v in attn_specs.items() if not isinstance(v, MLAAttentionSpec)
    }
    attn_specs = {k: v for k, v in attn_specs.items() if k not in draft_specs}
    if not mamba_specs or not attn_specs or not all(
        type(s) is MLAAttentionSpec for s in attn_specs.values()
    ):
        return None
    if not all(isinstance(s, AttentionSpec) for s in draft_specs.values()):
        return None
...
class DraftSlidingWindowSpec(SlidingWindowSpec):
    """DFLASH2-PATCH: a drafter's sliding-window layer on the GLM-5-Next layout.

    Identical to SlidingWindowSpec except that it opts out of prefix caching.
    That opt-out is what lets the draft group keep a small block size (its
    tensors are per-layer, sized page * num_blocks, so the page must be small
    or the drafter eats the KV budget) without dragging anything else down:
    engine/core.py takes `cache_config.block_size = min(...)` and
    kv_cache_utils takes the hash block as the gcd, both over groups that
    participate in prefix caching only; the scheduler block is the lcm over
    all groups, which stays at the target's as long as this block divides it
    (enforced in _glm5_next_draft_groups). The kpool tail uses the same
    mechanism for the same reason.

    What is given up: after a prefix-cache hit the target skips the cached
    tokens, so the drafter never writes context K/V for them
    (precompute_and_store_context_kv only covers the tokens the target ran)
    and its freshly allocated blocks hold stale data. Drafts for such a request
    attend to garbage until 2048 (the window) new tokens have been generated;
    they are still verified, so output is unaffected, only acceptance length.
    With participation (the previous, block-2304 variant) the draft blocks for
    the cached prefix would have been shared and valid. That trade is
    documented in NOTES.md; the block size is the lever for it.
    """
    ...
    def is_uniform_with_collection(
        ...
        return all(
            isinstance(spec, DraftSlidingWindowSpec)
            and spec.sliding_window == self.sliding_window
            for spec in kv_cache_specs.values()
        )


_DRAFT_SW_MANAGER_CLS: type | None = None


def _ensure_draft_sliding_window_spec_registered() -> None:
    """DFLASH2-PATCH: register DraftSlidingWindowSpec with a manager that keeps
    SlidingWindowManager's window eviction but never touches the prefix cache
    (the KpoolTailManager pattern). Lazy: single_type_kv_cache_manager imports
    this module at its top, so the import must not be module-level here. Runs
    in the engine-core process before the scheduler's coordinator is built;
    workers only unpickle the spec class and never look it up."""
    global _DRAFT_SW_MANAGER_CLS
    if _DRAFT_SW_MANAGER_CLS is None:
        from vllm.v1.core.single_type_kv_cache_manager import SlidingWindowManager

        class DraftSlidingWindowManager(SlidingWindowManager):
            ...
            def find_longest_cache_hit(
                ...
                # Never reached through HybridKVCacheCoordinator (the group is
                # skipped in verify_and_split_kv_cache_groups); safe answer if
                # some other path asks.
                return tuple([] for _ in kv_cache_group_ids), 0

            def cache_blocks(self, request, num_tokens, retention_interval=None):
                # Never hash draft blocks: their content is per request and
                # the hashes are computed at the target's granularity anyway.
                return

            def get_num_common_prefix_blocks(self, running_request_id):
                return 0

        _DRAFT_SW_MANAGER_CLS = DraftSlidingWindowManager
    KVCacheSpecRegistry.register(
        DraftSlidingWindowSpec,
        _DRAFT_SW_MANAGER_CLS,
        uniform_type_base_spec=DraftSlidingWindowSpec,
    )


def _glm5_next_draft_block_size(spec: AttentionSpec, target_block_size: int) -> int:
    """DFLASH2-PATCH: block size for a drafter sliding-window group.

    `Attention.get_kv_cache_spec` reports the backend's smallest kernel block
    (16 for FlashAttention/FlexAttention/Triton and FlashInfer) and expects the
    grouper to scale it. The block must be a multiple of that kernel block and
    a divisor of the target's block, so the scheduler block (lcm over all
    groups) stays the target's.

    Default: the largest such divisor that is at most a quarter of the target
    block (576 for --block-size 2304, 512 for 2048). Override with
    VLLM_GLM5NEXT_DRAFT_BLOCK_SIZE (exact, must divide). Why not as small as
    possible: every pool block a draft layer holds also reserves that block's
    MLA/indexer/mamba pages, and the sliding-window admission cap is
    cdiv(window - 1 + max_in_flight_tokens, block) + 1 blocks per request
    (41 at 256, 19 at 576 with 8192 in flight), so a small block trades tensor
    bytes for pool-block demand during prefill. A quarter of the target block
    is the balance point for this model (numbers in NOTES.md).
    """
    native = spec.block_size
    requested = os.environ.get("VLLM_GLM5NEXT_DRAFT_BLOCK_SIZE")
    if requested is None:
        limit = max(native, target_block_size // 4)
        candidates = [
            b
            for b in range(native, limit + 1, native)
            if target_block_size % b == 0
        ]
        if not candidates:
            raise NotImplementedError(
                f"no draft block size in [{native}, {limit}] divides the target "
                f"block size {target_block_size}; set VLLM_GLM5NEXT_DRAFT_BLOCK_SIZE"
            )
        return max(candidates)
    block = int(requested)
    if block % native != 0 or target_block_size % block != 0:
        raise NotImplementedError(
            f"VLLM_GLM5NEXT_DRAFT_BLOCK_SIZE={block} must be a multiple of the "
            f"draft kernel block {native} and a divisor of the target block size "
            f"{target_block_size}"
        )
    return block
...
def _glm5_next_draft_groups(
    draft_specs: dict[str, KVCacheSpec], block_size: int
) -> list[KVCacheGroupSpec]:
    """DFLASH2-PATCH: KV cache groups for drafter attention layers riding on
    the GLM-5-Next layout.

    Each draft layer gets its own tensor (`get_kv_cache_config_from_groups`),
    sized page * num_blocks like every other layer, so the draft page must be
    small: sliding-window draft layers become DraftSlidingWindowSpec at a
    small block (see _glm5_next_draft_block_size) and opt out of prefix
    caching (see the spec's docstring for why that is the only way to keep
    the small block from lowering the whole model's scheduler block and hash
    granularity). A full-attention drafter layer (EAGLE3 without a window)
    keeps participating and is raised to the target's block size, as before.
    """
    if not draft_specs:
        return []
    ...
    for name, spec in draft_specs.items():
        if isinstance(spec, SlidingWindowSpec) and not isinstance(
            ...
            _ensure_draft_sliding_window_spec_registered()
            scaled[name] = replace_as(
                spec,
                DraftSlidingWindowSpec,
                block_size=_glm5_next_draft_block_size(spec, block_size),
            )
            continue
        if block_size % spec.block_size != 0:
            raise NotImplementedError(
                f"Layer {name}: draft block size {spec.block_size} does not "
                f"divide the target block size {block_size}; pick a "
                "--block-size that is a multiple of the draft backend's kernel "
                "block."
            )
        scaled[name] = (
            spec if spec.block_size == block_size else replace(spec, block_size=block_size)
        )
...
def _glm5_next_draft_bytes_per_block(draft_groups: list[KVCacheGroupSpec]) -> int:
    """DFLASH2-PATCH: bytes one pool block costs across all draft layers."""
    return sum(
        len(g.layer_names) * g.kv_cache_spec.page_size_bytes for g in draft_groups
    )
...
def _glm5_next_tensor_layout(
    ...
    # DFLASH2-PATCH: drafter groups (see _glm5_next_draft_groups) are plain
    # AttentionSpec groups; the tail is a UniformTypeKVCacheSpecs group here,
    # so nothing of the target's is misclassified. A PP stage without the
    # drafter sees the group with an empty layer list; it still counts.
    draft_groups = [
        g
        for g in kv_cache_groups
        if isinstance(g.kv_cache_spec, AttentionSpec)
        and not isinstance(g.kv_cache_spec, UniformTypeKVCacheSpecs)
    ]
...
def get_kv_cache_config_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        # DFLASH2-PATCH: draft layers are plain per-layer tensors on top.
        per_block = (
            len(mla_names) * mla_page
            + len(idx_names) * idx_page
            + _glm5_next_draft_bytes_per_block(draft_groups)
        ...
        kv_cache_tensors = [
            ...
            # DFLASH2-PATCH: one unshared tensor per draft layer.
            KVCacheTensor(
...
def _max_memory_usage_bytes_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        if tail_names:
            ...
            blocks_needed += 1
        # DFLASH2-PATCH: draft groups draw their (window-bounded) blocks from
