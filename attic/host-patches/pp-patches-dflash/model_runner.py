...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        if self.speculative_config is not None:
            ...
            if self.speculative_config.method in ("eagle3", "dflash", "dspark"):
                ...
                # DFLASH-PP-PATCH: the blanket "with pipeline parallel is not
                # supported" ValueError used to fire here. The aux states are
                # captured on whichever stage owns the layer, and the drafter
                # lives on the last stage, so PP only works if the model
                # itself forwards them through IntermediateTensors. That is a
                # property of the model class, which does not exist yet at
                # this point; the check moved to load_model() below.
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        ...
        with DeviceMemoryProfiler() as m:
            ...
            if self.use_aux_hidden_state_outputs:
                ...
                if self.use_pp and not getattr(
                    self.model, "supports_pp_aux_hidden_states", False
                ):
                    # DFLASH-PP-PATCH: see __init__. Same failure as before,
                    # just later and naming the model.
                    raise ValueError(
                        f"{self.speculative_config.method} with pipeline parallel "
                        f"is not supported: {type(self.model).__name__} does not "
                        "forward aux hidden states across PP stages "
                        "(supports_pp_aux_hidden_states)."
                    )
    ...
    def update_pp_decode_requests(self):
        ...
        if self.pp_handler is not None:
            ...
            if outputs is not None:
                # PP-PATCH: drafts the last rank proposed for these requests'
                # next step, written before prepare_inputs reads them into
                # input_ids.
                draft_tokens = outputs.pop("draft_tokens", None)
                draft_idx_mapping = outputs.pop("draft_idx_mapping", None)
    ...
    def sample_tokens(
        ...
        if self.speculator is not None:
            ...
            _pp_drafts = draft_tokens
            if self.adaptive_verification is not None:
                self.adaptive_verification.record_confidences(
                    self.speculator.draft_token_confidence_probs, input_batch
                )

        # PP-PATCH: the receiving ranks post this recv whenever
        # num_speculative_steps > 0, so the sender must ALWAYS match it. Warmup
        # drives decode steps that never reach propose(); skipping the send there
        # left the receivers in an unmatched collective and the NCCL watchdog
        # took the process down with a pp_broadcast timeout. Zeros are a correct
        # "no drafts" value: the rows are overwritten before they are read.
        if self.pp_handler is not None and self.num_speculative_steps > 0:
            if _pp_drafts is None:
                _pp_drafts = torch.zeros(
                    (input_batch.num_reqs, self.num_speculative_steps),
                    dtype=torch.int64,
                    device=self.device,
                )
            self.pp_handler.broadcast_draft(_pp_drafts)
