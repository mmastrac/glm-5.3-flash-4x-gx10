...
class Glm5NextModel(nn.Module, EagleModelMixin):
    ...
    # DFLASH-PP-PATCH: auxiliary hidden states for EAGLE3/DFlash drafters.
    #
    # An aux id L (as set by set_aux_hidden_state_layers, already +1-shifted
    # by eagle3_utils from the drafter's target_layer_ids) is the completed
    # output of global layer L-1. Under mHC the completed output of a non-final
    # layer is never materialised: the layer returns (x, residual, post, comb)
    # and defers its hc_post to the next layer's fused post+pre kernel. For an
    # aux layer we apply that hc_post standalone (same kernel the final layer
    # and the PP boundary use) and contract the n streams with hc_contract,
    # i.e. a mean over the stream axis. That is exactly what the DFlash
    # reference capture does for this model (sglang PR #36708:
    # hc_contract(hidden_states + residual, hc_mult)) and what DeepSeek V4's
    # dspark capture does here (mhc_post_tilelang(...).mean(dim=1)).
    #
    # Under PP the aux states of earlier stages ride along in
    # IntermediateTensors under "aux_hidden_states.<L>"; this rank prepends
    # them so the drafter always sees the list in ascending L order, which is
    # the order its fc was trained on. Nothing here is indexed by a local
    # layer number: the loop enumerates from self.start_layer.
    @staticmethod
    def aux_key(aux_id: int) -> str:
        return f"aux_hidden_states.{aux_id}"

    def _set_aux_hidden_state_layers(self, layers: tuple[int, ...]) -> None:
        num_layers = self.config.num_hidden_layers
        bad = [l for l in layers if not (1 <= l <= num_layers)]
        if bad:
            raise ValueError(
                f"aux hidden state layer ids {bad} out of range 1..{num_layers} "
                f"(ids are 1-based: id L is the output of layer L-1)"
            )
        self.aux_hidden_state_layers = tuple(sorted(layers))

    def incoming_aux_ids(self) -> tuple[int, ...]:
        """Aux ids produced by earlier PP stages (layer L-1 < start_layer)."""
        return tuple(
            l for l in self.aux_hidden_state_layers if l - 1 < self.start_layer
        )
    ...
    def _completed_layer_output(
        ...
        if post is None:
            # Non-mHC (70B) layer: hidden_states is already residual + mlp
            # output. Final mHC layer: already hc_post'ed and contracted.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if get_pp_group().is_first_rank:
            ...
            # PP-PATCH: the sending stage materialised its last layer's
            # deferred hc_post, so this is the post-mapped residual stream,
            # rank 3 [tokens, n_streams, hidden] under mHC (exactly the
            # residual_cur that MHCFusedPostPreOp hands the next layer). The
            # first layer here takes the post-is-None path, which feeds its
            # input to the standalone hc_pre and sets residual to it; a
            # non-mHC layer does the same with the plain hidden state.
        ...
        # DFLASH-PP-PATCH: earlier stages' aux states first (their ids are all
        # lower than anything this stage produces), then our own in layer order.
        aux_ids: list[int] = []
        aux_hidden_states: list[torch.Tensor] = []
        if self.aux_hidden_state_layers and not get_pp_group().is_first_rank:
            assert intermediate_tensors is not None
            for aux_id in self.incoming_aux_ids():
                aux_ids.append(aux_id)
                aux_hidden_states.append(intermediate_tensors[self.aux_key(aux_id)])
        ...
        for idx, layer in enumerate(self._active_layers, start=self.start_layer):
            ...
            if idx + 1 in self.aux_hidden_state_layers:
                ...
                aux_ids.append(idx + 1)
                aux_hidden_states.append(aux)

        if not get_pp_group().is_last_rank:
            # PP-PATCH: post/comb are the deferred hc_post of this stage's last
            # mHC layer. Apply it here, exactly as the final layer does before
            # contracting, and ship the resulting post-mapped residual stream,
            # rank 3 [tokens, n_streams, hidden] bf16 (mhc_post_tilelang
            # returns empty_like(residual)). No contract: only layer 44 folds
            # the streams, and the next stage's hc_pre needs all n of them.
            # Without this the next stage would take the MLP output as the
            # stream and the boundary layer's residual mix would never happen.
            if post is not None:
                hidden_states = self._active_layers[-1].hc_post(
                    hidden_states, residual, post, comb
                )
            # DFLASH-PP-PATCH: aux states collected so far (incoming + ours)
            # ride along in the same dict; the next stage's
            # make_empty_intermediate_tensors allocates exactly these keys.
            tensors = {"hidden_states": hidden_states}
            for aux_id, aux in zip(aux_ids, aux_hidden_states):
                tensors[self.aux_key(aux_id)] = aux
        ...
        if self.aux_hidden_state_layers:
            # DFLASH-PP-PATCH: the runner unpacks a tuple whenever
            # use_aux_hidden_state_outputs is set, which is exactly when
            # set_aux_hidden_state_layers was called.
            assert len(aux_hidden_states) == len(self.aux_hidden_state_layers), (
                f"collected aux ids {aux_ids}, configured "
...
class Glm5NextForCausalLM(
    ...
    # DFLASH-PP-PATCH: read by the model runner in place of its blanket
    # "eagle3/dflash with pipeline parallel is not supported" guard. True means
    # this model forwards earlier stages' aux hidden states through
    # IntermediateTensors (see Glm5NextModel.forward).
    supports_pp_aux_hidden_states: ClassVar[bool] = True
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # PP-PATCH: the stage boundary carries the post-mapped mHC residual
        # stream. That stream is RANK 3, [tokens, n_streams, hidden_size]:
        # hc_expand builds it as unsqueeze(1).expand(-1, n, -1) (layers/mhc.py
        # hc_expand), mhc_pre_tilelang requires residual of shape
        # (..., hc_mult, hidden_size) in bf16 and reads hc_mult from
        # shape[-2] (kernels/mhc/tilelang.py mhc_pre_tilelang), and
        # mhc_post_tilelang returns empty_like(residual). A flat
        # [tokens, n * hidden] buffer makes hc_mult equal the token count and
        # trips `assert fn.shape[0] == hc_mult3`. A non-mHC (70B) config
        # carries the plain [tokens, hidden_size] hidden state.
        _hidden = self.config.hidden_size
        _n_streams = (
            int(getattr(self.config, "mhc_num_residual_streams", 0) or 0)
        ...
        def _make_empty_intermediate_tensors(
            ...
            shape = (
                (batch_size, _n_streams, _hidden)
                if _n_streams
                else (batch_size, _hidden)
            )
            tensors = {
                "hidden_states": torch.zeros(shape, dtype=dtype, device=device)
            }
            # DFLASH-PP-PATCH: one contracted [tokens, hidden] bf16 buffer per
            # aux state produced by an earlier stage. Read at call time, not
            # at construction: the runner calls set_aux_hidden_state_layers
            # after load_model() and only then allocates this buffer, and the
            # runner copies received tensors by iterating THESE keys, so the
            # set here must equal what the previous stage ships.
            for aux_id in _model.incoming_aux_ids():
                tensors[_model.aux_key(aux_id)] = torch.zeros(
...
class Glm5NextForConditionalGeneration(
    ...
    # DFLASH-PP-PATCH: see Glm5NextForCausalLM. SupportsEagle3's
    # set_aux_hidden_state_layers resolves through self.language_model.model,
    # which is the Glm5NextModel above.
    supports_pp_aux_hidden_states: ClassVar[bool] = True
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # PP-PATCH: alias the language model's factory, as
        # Glm4vForConditionalGeneration.__init__ (skipped above) would have.
